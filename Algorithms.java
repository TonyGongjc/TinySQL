...
/**
 * Created by anderson on 12/4/15.
...
public abstract class Algorithms {
    ...
    public static HashSet<String> tableInExpression(Statement statement) {
        statement.containsTable = new HashSet<>();
        if(statement.getAttribute().equalsIgnoreCase("COL_ID")) {
            assert statement.getBranches().size() > 1;
            statement.containsTable.add(statement.getBranches().get(0).getAttribute());
            return statement.containsTable;
        } else {
            if(statement.getBranches()==null||statement.getBranches().size()==0) {
                return statement.containsTable;
            }else {
                for (Statement subStatement: statement.getBranches()) {
                    statement.containsTable.addAll(tableInExpression(subStatement));
                }
                return statement.containsTable;
            }
        }
     ...
     */
    public static void mergeField(Statement statement) {
        /**
         * This function is for mergeField of several table, in a statement.
         */

        if (statement.getAttribute().equalsIgnoreCase("COL_ID")) {
            List<Statement> cols = statement.getBranches();
            StringBuilder s = new StringBuilder();
            List<Statement> tempBranches = new ArrayList<>();
            for (Statement field: cols) {
                s.append(field.getAttribute()+".");
            }
            s.deleteCharAt(s.length()-1);
            tempBranches.add(new Statement(s.toString(), true));
            statement.setBranches(tempBranches);
        } else {
            if(statement.getBranches()!=null&&statement.getBranches().size()>0) {
                for (Statement st : statement.getBranches()) {
                    mergeField(st);
                }
            }
        }
