...
//          System.out.println("REMOVE!\n");
  ...
  public static Relation filter(SchemaManager schema_manager, MainMemory mem,
                                 Relation inputRelation, Expression exp, ArrayList<String> fields, int mode){
  ...
  public static ArrayList<Tuple> twoPassNaturalJoin(SchemaManager schema_manager, MainMemory mem, String r1, String r2, final String field){
    ...
    while(!isEmpty(tuples1)&&!isEmpty(tuples2)) {
     ...
     /* System.out.println("SELECT: TWO PASS NATURAL JOIN NULL TEST\n");
      ...
      if(minVal1!=null){
         min1 = minVal1.getField(field).toString();
      }

      if(minVal2!=null){
        min2 = minVal2.getField(field).toString();
      }

      if(min1!=null&&min2!=null&&min1.equals(min2)){
        /* Get all the minimum tuples in both collections, cross join and output */
        int count1 = getMinCount(tuples1,field,min1);
        int count2 = getMinCount(tuples2,field,min2);

        ArrayList<Tuple> minTuples1 = getMinTuples(tuples1,field,min1);
        ArrayList<Tuple> minTuples2 = getMinTuples(tuples2,field,min2);
        ...
        * we have to keep the other relation in case for more joins */
        ...
        for(int i=0;i<segments1.size();i++){
          ...
          if(getBlockMinCount(tuples1.get(i),field,min1)==tmpblk.getNumTuples()){
            flag1 = true;
        ...
        for(int i=0;i<segments2.size();i++){
          ...
          if(getBlockMinCount(tuples2.get(i),field,min1)==tmpblk.getNumTuples()){
            flag2 = true;
